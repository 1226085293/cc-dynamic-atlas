...
class lib_string_extend {
    /** 获取字符块 */
    get_block(label_s_, head_s_ = "{", tail_s_ = "}") {
        /**返回数据 */
        let result_ss = [];
        /**前缀下标 */
        let head_index_n = 0;
        ...
        while (head_index_n !== -1 && tail_index_n !== -1) {
            ...
            while (head_count_n > 0) {
                ...
                // 更新前缀计数
                {
                    if ((next_head_index_n !== -1 && next_head_index_n < tail_index_n) ||
                        (tail_index_n === -1 && next_head_index_n !== -1)) {
                        ++head_count_n;
                        find_index_n = next_head_index_n + head_s_.length;
                    }
                    else if ((tail_index_n !== -1 && tail_index_n < next_head_index_n) ||
                        (tail_index_n !== -1 && next_head_index_n === -1)) {
                        --head_count_n;
    ...
    /** 项目路径转换为绝对路径 */
    project_path_to_fs_path(path_s_) {
        if (path_s_.length <= "project://".length) {
            return "";
        }
        return path_1.default.normalize(path_s_.replace("project:/", Editor.Project.path));
    }
    /** 绝对路径转换为项目路径 */
    fs_path_to_project_path(path_s_) {
        return path_1.default.normalize(path_1.default.normalize(path_s_).replace(Editor.Project.path, "project:"));
    }
    /** db 路径转换为绝对路径 */
    db_path_to_fs_path(path_s_) {
        if (path_s_.length <= "db://".length) {
            return "";
        }
        return path_1.default.normalize(path_s_.replace("db:/", Editor.Project.path));
    }
    /** 绝对路径转换为 db 路径 */
    fs_path_to_db_path(path_s_) {
        return path_1.default.normalize(path_s_).replace(Editor.Project.path, "db:/").replace(/\\/g, "/");
    }
}
exports.default = new lib_string_extend();
